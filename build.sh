...
# Mono.Cecil 0.10+ is required to support the "portable pdb format", which is generated by mono 5 (but we opt out below at the moment)
curl -sS https://api.nuget.org/packages/mono.cecil.0.10.0-beta6.nupkg > /tmp/mono.cecil.0.10.0-beta6.nupkg.zip
unzip /tmp/mono.cecil.0.10.0-beta6.nupkg.zip -d /tmp/cecil
...
xbuild /p:Configuration=Debug /p:CscToolExe=mcs.exe clrzmq4.mono.sln
